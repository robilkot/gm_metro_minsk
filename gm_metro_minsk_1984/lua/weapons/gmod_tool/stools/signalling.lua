...
local TypesOfSignal = {"Inside","Outside big","Outside small","Small","Station box"}
...
function TOOL:SpawnSignal(ply,trace,param)
    ...
    if param == 2 then
        ...
        if IsValid(ent) then
            ...
            ent.ARSOnly = self.Signal.ARSOnly or (self.Signal.Type == 5)
...
function TOOL:BuildCPanelCustom()
    ...
    if tool.Type == 1 then
        if (tool.Signal.Type == 5) then
            tool.Signal.Left = true
        end
        ...
        if not tool.Signal.ARSOnly and tool.Signal.Type != 5 then
        ...
        if tool.Signal.Type == 1 then
            local VRoutT,VRoutN = CPanel:TextEntry("Custom route number:")
                ...
                function VRoutT:OnChange()
                    ...
                    local NewValue = ""
                    for i = 1,#oldval do
                        NewValue = NewValue..((oldval[i] or ""):upper():match("[1-4DWKFLRXP]+") or "")
                    end
        ...
        local VRouT,VRouN = CPanel:TextEntry("Route number:")
                ...
                function VRouT:OnChange()
                    ...
                    for i = 1,#oldval do
                        ...
                        NewValue = NewValue..((oldval[i] or ""):upper():match(tool.Signal.Type == 1 and "[%dDFLRP]" or "[%dDP]") or "")
                    end
                    self:SetText(NewValue)
                    self:SetCaretPos(0)
                end
        ...
        if (tool.Signal.Type != 2 and tool.Signal.Type != 4 and tool.Signal.Type != 5) then
