...
function ENT:SpawnLetter(i,model,pos,letter,double,angle)
    ...
    if double ~= false and not IsValid(self.Models[2][i]) and (self.Double or not self.Left) and (not letter:match("s[1-3]") or letter == "s3" or self.Double and self.Left) then
        ...
        self.Models[2][i]:SetAngles(self:LocalToWorldAngles(Angle(0,180,0) + angle))
    ...
    if not double and not IsValid(self.Models[2][i.."d"]) and (self.Double or self.Left) and (not letter:match("s[1-3]") or letter == "s3" or self.Double and not self.Left) then
        ...
        self.Models[2][i.."d"]:SetAngles(self:LocalToWorldAngles(Angle(0,180,0) + angle))
...
net.Receive("metrostroi-signal", function()
    ...
    ent.LightType = net.ReadInt(4) -- количество типов светофоров
    ...
    if not self.ModelsCreated then
        ...
        if not self.ARSOnly then
            ...
            if self.Arrow then
                ...
                self.Models[1]["q"]:SetPos(self:LocalToWorld(self.SpecRouteNumbers.pos))
            ...
            local offset = self.TrafficLightModels[4].name 
            local angle = (self.LightType == 4) and self.TrafficLightModels[4].name_s_ang or Angle(0, 0, 0)
            
            name = string.Replace(name, " ", "")
            name = string.Replace(name, "/", "")
            if (self.Left) then name = string.reverse(name) end

            if (self.LightType == 4) then
                offset = self.TrafficLightModels[4].name_s
            elseif (self.LightType == 2) then
                offset = self.TrafficLightModels[4].name_out
            end
            
            if (self.Left or self.LightType == 4) then offset = offset - Vector(10, 0, 0) end
            offset = offset - Vector((5.85/2) * (3 - (#name)), 0, 0)

            for i = 0, #name-1 do
                local id = i
                self:SpawnLetter(i, "models/metrostroi/signals/mus/sign_letter.mdl", offset - Vector(id*5.85,0,0),(Metrostroi.LiterWarper[name[i+1]] or name[i+1]), not self.Left and true or false, angle)
            end
        ...
        for k,v in pairs(self.RouteNumbers) do
            ...
            if not IsValid(self.Models[3]["rou1"..k]) and State1 > 0 then
                ...
                self.Models[3]["rou1"..k]:SetSkin(v[3] and self.OldRouteNumberSetup[5][v[1]] or self.OldRouteNumberSetup[7][v[2]] or self.OldRouteNumberSetup[6][v[1]] or tonumber(v[1])-1)
            ...
            if not IsValid(self.Models[3]["rou2"..k]) and v[3] and v[2] and State2 > 0 then
                ...
                self.Models[3]["rou2"..k]:SetSkin(self.OldRouteNumberSetup[5][v[2]] or self.OldRouteNumberSetup[7][v[2]] or tonumber(v[2])-1)
