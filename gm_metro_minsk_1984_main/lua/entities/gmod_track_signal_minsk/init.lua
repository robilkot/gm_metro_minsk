...
function ENT:Initialize()
	self:SetModel("models/mn_r/mn_r_joint1.mdl")
	self.Sprites = {}
	self.Sig = ""
...
function ENT:PostInitalize()
	...
	if self.SignalType == 4 then
		self:SetModel(self.Left and "models/mn_r/mn_r_joint4.mdl" or "models/mn_r/mn_r_joint3.mdl")
	else
		self:SetModel(self.Left and "models/mn_r/mn_r_joint2.mdl" or "models/mn_r/mn_r_joint1.mdl")
	end
...
function ENT:Think()
	...
	--DEBUG
	if SendAutomaticRC then
		SendAutomaticRC(self)
	end
	...
	if not self.ControllerLogic then
		...
		if (CurTime() - self.PrevTime) > 1.0 then
			self.PrevTime = CurTime()+math.random(0.5,1.5)
			--self:ARSLogic(self.PrevTime - CurTime())
			--self:CheckOccupation() ИЗМЕНЕНИЯ ДЛЯ ПУЛЬТА
			if self.Node and  self.TrackPosition then
				self.Occupied,self.OccupiedBy,self.OccupiedByNow = Metrostroi.IsTrackOccupied(self.Node, self.TrackPosition.x,self.TrackPosition.forward,self.ARSOnly and "ars" or "light", self)
			end
		end
...
function ENT:SendUpdate(ply)
	net.Start("metrostroi-signal")
		...
		net.WriteInt(self.SignalType or 0,4) --количество светофоров
